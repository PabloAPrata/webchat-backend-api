...
public interface PatientRepository extends JpaRepository<Patient, Long> {
    ...
    @Query("SELECT u FROM User u WHERE " + "(:name IS NULL OR LOWER(u.name) LIKE LOWER(CONCAT('%', :name, '%'))) " + "AND (:email IS NULL OR LOWER(u.email) LIKE LOWER(CONCAT('%', :email, '%'))) " + "AND (:phone IS NULL OR u.telephone LIKE CONCAT('%', :phone, '%')) " + "AND (:cpf IS NULL OR u.cpf = :cpf)")
    List<User> searchUsers(@Param("name") String name, @Param("email") String email, @Param("phone") String phone, @Param("cpf") String cpf);
